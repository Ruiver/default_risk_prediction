    ...
    with open("result_new.txt", 'r', encoding='utf8') as f:
        for line in f.readlines():
            ...
            if '\1' in line:
                ...
                try:
                    if eval(eva_ent.split("EVA: ")[1].split(' ALL')[0]):
...
def get_platAndproblemPlat():
    ...
    # collection = pymongo.MongoClient(host='10.205.27.73')['lunwen']['problem']
...
def get_x_y(plat_word):
    ...
    for key, value in plat_word.items():
        ...
        if key in problemPlat:
            y.append([0,1])
            temp_n.append(key + '\t' + value)
        else:
            y.append([1,0])
            temp_p.append(key + '\t' + value)
    print("positive is ")
    with open('positive', 'w', encoding='utf8') as f:
        for i in temp_p:
            f.write(i + '\n')
    print("negative is ")
    with open('negative', 'w', encoding='utf8') as f:
        for i in temp_n:
            f.write(i + '\n')
...
def get_LSTM(train_x, train_y, train_len, test_x, test_y, test_len, word2id):
    ...
    layer2 = tf.layers.dense(output, 100)
    layer3 = tf.layers.dense(layer2, 100)
    out = tf.layers.dense(layer3, 2)
    out = tf.nn.softmax(out)
    pred = tf.cast(tf.argmax(out, dimension=1), tf.int32)
    #pred = tf.cast(tf.greater(out, 0.4), tf.int32)
    #loss = tf.losses.mean_squared_error(tf_y, out)
    #loss = tf.losses.softmax_cross_entropy(tf_y, out)
    cross_entropy = tf.reduce_mean(-0.7 * tf.reduce_sum(tf_y[:, 0] * tf.log(out[:, 0])) - 0.1 * tf.reduce_sum(tf_y[:, 1] * tf.log(out[:, 1])))
    # loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf_y, logits=out))
    optimizer = tf.train.AdamOptimizer()
    ...
    for step in range(150):
        # train and net output
        _, l, train_pred_y_ = sess.run([train_op, cross_entropy, pred], {tf_x: train_x, tf_y: train_y, sequence_lengths: train_len, dropout_pl: 0.5})
        if step % 10 == 0:
            print("step is:", step, "loss is:", l)
            print(classification_report(np.argmax(train_y,axis=1), train_pred_y_))
            print("train_accuary:", get_accuacy(np.argmax(train_y,axis=1), train_pred_y_))
    print("----------test result----------")
    print("-------------------------------")
    print("-------------------------------")
    ...
    # print(pred_y,test_y)
    print(classification_report(np.argmax(test_y, axis=1), pred_y_, digits=4))
    print("accuary:", get_accuacy(np.argmax(test_y, axis=1), pred_y_))
    print("auc:", roc_auc_score(np.argmax(test_y, axis=1), np.argmax(out_, axis=1)))
