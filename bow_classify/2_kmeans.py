    ...
    with open("result_new.txt", 'r', encoding='utf8') as f:
        for line in f.readlines():
            ...
            if '\1' in line:
                ...
                try:
                    # plat_word[key] += eval(eva_ent.split("EVA: ")[1])
...
def get_vec_y(plat_word):
    ...
    return plat_vec
def get_biaozhu():
    with open("./fc_test/all_label", "r") as f:
        plats, problem_plats = [], []
        for line in f.readlines():
            spl = line.strip().split("\t")
            plats.append(spl[0])
            if (int(spl[2]) == 1) :
                problem_plats.append(spl[0])
    return plats, problem_plats
def get_vec_y_label(plat_word):
    plat_vec = {}
    plats, problemPlat = get_biaozhu()
    plats_real, problemPlat_real = get_platAndproblemPlat()
    ...
    for key, value in plat_word.items():
        ...
        for i in x:
            vec += i
        vec = vec/len(x)
        if key not in plats or key not in plats_real:
            continue
        if key in problemPlat:
            if key in problemPlat_real:
                plat_vec[key] = [vec, "11"]
            else:
                plat_vec[key] = [vec, "10"]
        else:
...
def get_dense(train_x, train_y, test_x, test_y, k_cluster):
    tf_x = tf.placeholder(tf.float32, (None, len(train_x[0])))  # input x
