...
def get_dense(train_x, train_y, test_x, test_y, words):
    tf_x = tf.placeholder(tf.float32, (None, len(words)))  # input x
    ...
    layer5 = tf.layers.dense(layer3, len(words), activation=tf.nn.relu6)
    ...
    print( classification_report(test_y, a_) )
    print("accuary:",get_accuacy(test_y, a_))
...
if __name__ == '__main__':
    ...
    plat_word = get_data()
    plat_vec = get_x_y(words, plat_word)
    ...
    get_dense(train_x, train_y, test_x, test_y, words)
